...
public class CapitalTradeOrderService {
    ...
    public String record(TransactionContext transactionContext, CapitalTradeOrderDto tradeOrderDto) {
        logger.info("远程调用Capital-record方法---start---");
        ...
        try {
            ...
            HttpPost post = new HttpPost("http://127.0.0.1:7001/recordCap");
            post.addHeader("Content-Type", "application/json");
            StringEntity stringEntity = new StringEntity(objectMapper.writeValueAsString(request));
            post.setEntity(stringEntity);
            CloseableHttpResponse response = closeableHttpClient.execute(post);
            if (response.getStatusLine().getStatusCode() != 200) {
                throw new NoWarnException("远程调用Capital-record方法异常，返回code非200");
            }
